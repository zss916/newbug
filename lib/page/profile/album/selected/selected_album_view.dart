...
class SelectedAlbumView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: GetBuilder<SelectedAlbumLogic>(
        init: SelectedAlbumLogic(),
        builder: (logic) {
          return Column(
            children: [
              Expanded(
                child: SingleChildScrollView(
                  child: Column(
                    children: [
                      if (logic.images.isNotEmpty)
                        BuildPhotosWidgetPlus(
                          images: logic.images,
                          logic: logic,
                        ),
                      if (logic.images.isNotEmpty)
                        Divider(height: 6.h, color: Colors.transparent),
                      if (logic.videos.isNotEmpty)
                        BuildVideosWidgetPlus(
                          videos: logic.videos,
                          logic: logic,
                        ),
                      if (logic.videos.isNotEmpty)
