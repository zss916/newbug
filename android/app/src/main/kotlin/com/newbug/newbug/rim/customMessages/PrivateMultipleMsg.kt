...
@MessageTag(value = MessageIdentifiers.packagePrivacy, flag = MessageTag.ISCOUNTED)
class PrivateMultipleMsg : MediaMessageContent {
    // 自定义消息变量，可以有多个
    ...
    companion object {
        private const val TAG = "PrivateMultipleMsg"

        // 快速构建消息对象方法
        fun obtain(content: String?): PrivateMultipleMsg {
            val msg = PrivateMultipleMsg()
            msg.content = content
            return msg
        }

        @JvmField
        val CREATOR: Parcelable.Creator<PrivateMultipleMsg?> = object : Parcelable.Creator<PrivateMultipleMsg?> {
            override fun createFromParcel(source: Parcel): PrivateMultipleMsg? {
                return PrivateMultipleMsg(source)
            }

            override fun newArray(size: Int): Array<PrivateMultipleMsg?> {
