...
@MessageTag(value = MessageIdentifiers.system, flag = MessageTag.STATUS)
class SystemActionMsg : MediaMessageContent {
    // 自定义消息变量，可以有多个
    ...
    companion object {
        private const val TAG = "SystemActionMsg"

        // 快速构建消息对象方法
        fun obtain(content: String?): SystemActionMsg {
            val msg = SystemActionMsg()
            msg.content = content
            return msg
        }

        @JvmField
        val CREATOR: Parcelable.Creator<SystemActionMsg?> = object : Parcelable.Creator<SystemActionMsg?> {
            override fun createFromParcel(source: Parcel): SystemActionMsg? {
                return SystemActionMsg(source)
            }

            override fun newArray(size: Int): Array<SystemActionMsg?> {
