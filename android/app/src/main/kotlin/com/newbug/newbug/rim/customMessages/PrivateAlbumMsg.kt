...
@MessageTag(value = MessageIdentifiers.privacy, flag = MessageTag.ISCOUNTED)
class PrivateAlbumMsg : MediaMessageContent {
    // 自定义消息变量，可以有多个
    ...
    companion object {
        ...
        // 快速构建消息对象方法
        fun obtain(content: String?): PrivateAlbumMsg {
            val msg = PrivateAlbumMsg()
            msg.content = content
            return msg
        }

        @JvmField
        val CREATOR: Parcelable.Creator<PrivateAlbumMsg?> = object : Parcelable.Creator<PrivateAlbumMsg?> {
            override fun createFromParcel(source: Parcel): PrivateAlbumMsg? {
                return PrivateAlbumMsg(source)
            }

            override fun newArray(size: Int): Array<PrivateAlbumMsg?> {
