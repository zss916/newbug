...
@MessageTag(value = MessageIdentifiers.systemTip, flag = MessageTag.ISPERSISTED)
class SystemTipMsg : MediaMessageContent {
    // 自定义消息变量，可以有多个
    ...
    companion object {
        private const val TAG = "SystemTipMsg"

        // 快速构建消息对象方法
        fun obtain(content: String?): SystemTipMsg {
            val msg = SystemTipMsg()
            msg.content = content
            return msg
        }

        @JvmField
        val CREATOR: Parcelable.Creator<SystemTipMsg?> = object : Parcelable.Creator<SystemTipMsg?> {
            override fun createFromParcel(source: Parcel): SystemTipMsg? {
                return SystemTipMsg(source)
            }

            override fun newArray(size: Int): Array<SystemTipMsg?> {
