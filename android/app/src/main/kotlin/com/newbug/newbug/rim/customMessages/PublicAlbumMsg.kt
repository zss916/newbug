...
@MessageTag(value = MessageIdentifiers.public, flag = MessageTag.ISCOUNTED)
class PublicAlbumMsg : MediaMessageContent {
    // 自定义消息变量，可以有多个
    ...
    companion object {
        private const val TAG = "PublicAlbumMsg"

        // 快速构建消息对象方法
        fun obtain(content: String?): PublicAlbumMsg {
            val msg = PublicAlbumMsg()
            msg.content = content
            return msg
        }

        @JvmField
        val CREATOR: Parcelable.Creator<PublicAlbumMsg?> = object : Parcelable.Creator<PublicAlbumMsg?> {
            override fun createFromParcel(source: Parcel): PublicAlbumMsg? {
                return PublicAlbumMsg(source)
            }

            override fun newArray(size: Int): Array<PublicAlbumMsg?> {
